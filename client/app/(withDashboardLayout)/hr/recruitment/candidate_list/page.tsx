...
const NoticePage = () => {
  const [tableAllData, setTableData] =
    useState<TTableData[]>(candidateTableData);
...
const handleNewData:SubmitHandler<FieldValues>=async()=>{
  ...
  const HandleEditNotice = (id: number | string) => {
    ...
    console.log(id);
    const editTableData = tableAllData.find((data) => data.id === id);
    if (editTableData) {
  ...
  const HandleNoticeDelete = (id: number | string) => {
    const updateTableData: TTableData[] = tableAllData.filter(
      (data) => data.id !== id
    );
    if (updateTableData) {
  ...
  return (
    <div className="bg-white w-full min-h-screen rounded-2xl p-4 ">
            ...
            return (
              ...
              >
                ...
                <HRInput
                  ...
                  // required="required"
                  defaultValue={
                    editTableData[inputField?.key as keyof TTableData] || ""
                  }
                />
              </div>
            );
      ...
      {/* All Notice */}
      <HRTable tableHeader={candidateTableHeaders}>
            ...
            >
              ...
              <td className="w-1/6 border-r border-gray-200 px-3">
                <ul className="flex gap-2 items-center  p-2 ">
                  <li
                    onClick={() => HandleEditNotice(data?.id)}
                  ...
                  <li
                    ...
                    onClick={() => HandleNoticeDelete(data?.id)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
...
//     setIsOpen(!modalIsOpen);
//     console.log(id);
//     const editData=tableAllData.find(notice=>notice.id===id)
//     if(editData){
...
//   };
//   const HandleNoticeDelete = (id: number | string) => {
//     const updateNoticeData = tableAllData.filter((notice) => notice.id !== id);
//     setTableData(updateNoticeData);
//   };
//   return (
//     <div className="bg-white w-full min-h-screen rounded-2xl p-4 ">
//       {/* Notice Top Bar */}
...
// </Button>
// </div>
//         <SearchBox HandleSearchData={handleSearchNotice}></SearchBox>
//       </div>
